...
userRoutes.controller('RoutesController', ['$scope', '$http', '$compile', function($scope, $http, $compile){
    ...
    $scope.showMyRoutes = function(){
        ...
        if(myRoutesArr == null || myRoutesArr == "[]" || myRoutesArr.length == 0) {
            ...
            for(var i = 0; i<myRoutesArr.length; i++){
                ...
                route+='<img class="dots" src="../images/DOTS.png" ng-click="openIcons("icons+'+ myRoutesArr[i].trip_id +'")"><div class="iconsWrap hidden" id="icons+'+ myRoutesArr[i].trip_id +'"><button class = "editBtn"></button> <button class = "deleteBtn" ng-click="deleteRoute(' + myRoutesArr[i].trip_id + ')"></button><button class = "shareBtn"></button></div>'
    ...
    $scope.openIcons = function(iconsId){
        console.log("iconsId");
        var iconsWrap = angular.element(document.querySelector('#'+iconsId));
